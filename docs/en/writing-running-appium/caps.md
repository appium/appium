...
|`disableWindowAnimation`|Set device animation scale zero if the value is `true`. In session deletion process, Appium sets the animation scale as before the state of session creation. Defaults to `false` | `true`, `false` |
