...
|defaultImageTemplateScale| Appium resizes a template image with arbitrary scale by default if you set this setting to `4.0`, for example. Defaults to `1.0` (Does not scale). Storing scaled template images might help decreasing your storage size than storing original images. e.g. A user has `270 × 32 pixels` template image oridinally `1080 × 126 pixels`. The user stores the former image in his storage since he would like to minimise storage size. If Appium expands it to fourth, the template image can be almost the same as the original one. OpenCV can identify the scaled template image as the originally one. This setting helps the case. [appium-base-driver#307](https://github.com/appium/appium-base-driver/pull/307)|`0.5`, `10.0`, `100`| `1.0` |
