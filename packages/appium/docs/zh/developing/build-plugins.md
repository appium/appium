...
`updateServer` runs _after_ Appium's own routes are registered, so `expressApp.use(...)` here
...
is still _after_ Appium's own baseline middleware (CORS, WebSocket upgrade handling, body
