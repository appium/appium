...
| `includeNonModalElements` | Whether returns all of elements including no modal dialogs on iOS 13+. It fixes [cannot find elements on nested modal presentations](https://github.com/appium/appium/issues/13227), but it might make visibility attributes unreliable. Then, you can also enable `shouldUseTestManagerForVisibilityDetection` settings (defaults to `false`) to improve the visibility detection. Defaults to `false`. | `true`, `false` |1.15.0+|
