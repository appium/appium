...
androidCommon.prepareEmulator = function (cb) {
  if (this.args.avd !== null) {
    ...
    this.adb.getRunningAVD(avdName, function (err, runningAVD) {
      ...
      this.adb.autoDetectEmulatorType(this.args.avd, function (err, deviceModel) {
        if (err) return cb(err);
        deviceModel.launchAVD(this.args.avd, this.args.avdArgs, this.args.language,
          this.args.locale, this.args.avdLaunchTimeout, this.args.avdReadyTimeout, cb);
      }.bind(this));
    }.bind(this));
