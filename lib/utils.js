...
function parseCapsForInnerDriver (jsonwpCapabilities, w3cCapabilities, constraints = {}, defaultCapabilities = {}) {
  ...
  if (!_.isEmpty(defaultCapabilities)) {
    if (hasW3CCaps) {
      for (const defaultCapKey of _.keys(removeW3CPrefixes(defaultCapabilities))) {
        const firstMatch = w3cCapabilities.firstMatch || [];
        const alwaysMatch = w3cCapabilities.alwaysMatch || {};
        let isCapAlreadySet = false;
        for (const firstMatchEntry of firstMatch) {
          if (_.has(removeW3CPrefixes(firstMatchEntry), defaultCapKey)) {
            isCapAlreadySet = true;
            break;
          }
        }
        isCapAlreadySet = isCapAlreadySet || _.has(removeW3CPrefixes(alwaysMatch), defaultCapKey);
        if (!isCapAlreadySet) {
          // Only add the default capability if it is not overridden
          if (_.isArray(firstMatch) && !_.isEmpty(firstMatch)) {
            firstMatch[0] = {
              ...defaultCapabilities,
              ...firstMatch[0],
            };
          } else {
            w3cCapabilities.firstMatch = [defaultCapabilities];
          }
        }
      }
    }
