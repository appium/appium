...
| --------------- | ------- | --------- | ------- |
| iOS (including XCUITest) | Shut down sim after test. Do not destroy sim. Uninstall app from Simulator(1). Do not uninstall app from real device. | Uninstall app before and after real device test, destroy Simulator before and after sim test. They happen only _before_ if `resetOnSessionStartOnly: true` is provided | Do not destroy or shut down sim after test. Start tests running on whichever sim is running, or device is plugged in |
...
(1): Make sure the local data has been deleted from Simulator environment rather than they remian unexpectedly.
