    ...
    try {
      ...
      if (!_.isEmpty(this.args.denyInsecure)) {
        log.info(`Explicitly preventing use of insecure features ` +
                 `${JSON.stringify(this.args.allowInsecure)} for this server`);
        d.denyInsecure = this.args.denyInsecure;
      }

      if (!_.isEmpty(this.args.allowInsecure)) {
        log.info(`Enabling insecure features ${JSON.stringify(this.args.allowInsecure)} ` +
                 `for this server (unless denied by --deny-insecure)`);
