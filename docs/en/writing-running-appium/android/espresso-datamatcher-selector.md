...
Android apps have special types of Views called AdapterViews (e.g.: `ScrollView`, `ListView`, `GridView`) which don't render all of their child views but instead only render what's visible in the viewport. The data for the view is stored in memory in an "adapter" object and only the subset of that data is rendered as a view.

When using Espresso's Data Matcher, you can target views that are not on-screen by writing a [Hamcrest matcher](http://hamcrest.org/JavaHamcrest/javadoc/1.3/org/hamcrest/Matchers.html) that selects an item from the adapter. When that item is located, if the view is not visible in the View hierarchy, that view is immediately scrolled into the viewport.
...
This ListView displays menu items `Drag and Drop`, `Expandable Lists`, ... to `Lists`. This menu has several more items that aren't visible and can't be located with standard locators. For example, There's a menu item called `TextClock` that is not visible and that needs to be scrolled into view to be visible.

The `ListView` node in the above XML has an attribute called `adapters` that contains this data.
...
This lists all of the adapters that "back up" the `ListView` and can be targeted using a datamatcher selector.

Here's a code snippet that shows how to locate and click `TextClock`:
...
This selector is equivalent to writing this matcher in Espresso
...
In the above example, we select the parent `AdapterView` using an id selector and then select the child of that view. Locating the parent `AdapterView` is not necessary if the Activity only has one view. In that case, it can be omitted and it will default to the one view and we can write the selector like this:
