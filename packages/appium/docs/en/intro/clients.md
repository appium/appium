...
If the client disconnects before Appium finishes sending a `Create Session` response, Appium
deletes the new session once driver setup completes. Setup is not interrupted: the simulator may
still boot or the app may still install before cleanup runs. Retrying with the same
`X-Idempotency-Key` waits for setup and cleanup to finish, then starts one fresh request if the
original response was abandoned. Other retries with that key share the fresh response.
