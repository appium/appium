...
async function getJimpImage(data) {
  return await new B((resolve, reject) => {
    if (!_.isString(data) && !_.isBuffer(data)) {
      return reject(
        new Error('Must initialize jimp object with string or buffer')
      );
    }
    ...
    new Jimp(
      ...
      (err, imgObj) => {
        ...
        if (!imgObj) {
          return reject(
            new Error('Could not create jimp image from that data')
          );
        }
...
function getRectIntersection(rect, imageSize) {
  ...
  const width =
    imageSize.width >= left + rect.width ? rect.width : imageSize.width - left;
  const height =
    imageSize.height >= top + rect.height
      ? rect.height
