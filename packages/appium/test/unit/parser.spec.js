  ...
  describe('Server Parser', function () {
    describe('Appium arguments', function () {
      ...
      it('should parse --allow-insecure correctly', function () {
        ...
        p.parseArgs(['--allow-insecure', 'foo']).allowInsecure.should.eql([
          'foo',
        ]);
        p.parseArgs(['--allow-insecure', 'foo,bar']).allowInsecure.should.eql([
          'foo',
          'bar',
        ]);
        p.parseArgs(['--allow-insecure', 'foo ,bar']).allowInsecure.should.eql([
      ...
      it('should parse --deny-insecure correctly', function () {
        ...
        p.parseArgs(['--deny-insecure', 'foo']).denyInsecure.should.eql([
          'foo',
        ]);
        p.parseArgs(['--deny-insecure', 'foo,bar']).denyInsecure.should.eql([
          'foo',
          'bar',
        ]);
        p.parseArgs(['--deny-insecure', 'foo ,bar']).denyInsecure.should.eql([
      ...
      it('should parse --allow-insecure & --deny-insecure from files', function () {
        ...
        parsed.denyInsecure.should.eql([
          'nofeature1',
          'nofeature2',
          'nofeature3',
        ]);
      ...
      it('should allow multiple --use-drivers', function () {
        p.parseArgs(['--use-drivers', 'fake,phony']).useDrivers.should.eql([
          'fake',
          'phony',
        ]);
      });
      ...
      it('should respect --relaxed-security', function () {
        p.parseArgs(['--relaxed-security']).should.have.property(
    ...
    describe('extension arguments', function () {
      ...
      it('should parse driver args correctly from a string', async function () {
        ...
        const {config} = await readConfigFile(
          resolveFixture('config', 'driver-fake.config.json')
        );
        ...
        it('should error out', function () {
          (() =>
            p.parseArgs([
              '--driver-fake-silly-web-server-port',
              'foo',
            ])).should.throw(/must be integer/i);
        });
      });
