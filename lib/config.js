...
function getDeprecatedArgs (parser, args) {
  ...
  for (let rawArg of parser.raw_args) {
    let arg = rawArg[1].dest;
    let default = rawArg[1].default;
    let isDeprecated = !!rawArg[1].deprecated_for;
    if (args[arg] && args[arg] !== default && isDeprecated) {
      deprecated[rawArg[0]] = rawArg[1].deprecated_for;
    }
  }
  return deprecated;
}
