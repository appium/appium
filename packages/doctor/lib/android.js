...
class AndroidToolCheck extends DoctorCheck {
  constructor() {
    ...
    this.tools = [
      ...
      'android',
      'emulator',
      `apkanalyzer${system.isWindows() ? '.bat' : ''}`,
    ];
    this.noBinaries = [];
  }
  ...
  async diagnose() {
    ...
    for (const binary of this.tools) {
      try {
        log.info(
          `     '${binary}' is in ${await getAndroidBinaryPath(binary)}`
        );
      } catch (e) {
    ...
    if (this.noBinaries.length > 0) {
      return nok(
        `${this.noBinaries.join(', ')} could NOT be found in ${sdkRoot}!`
      );
    }
  ...
  fix() {
    if (typeof process.env.ANDROID_HOME === 'undefined') {
      return `Manually configure ${
        'ANDROID_HOME'.bold
      } and run appium-doctor again.`;
    }

    return (
      `Manually install ${this.noBinaries.join(', ').bold} and add it to ${
        'PATH'.bold
      }. ` +
