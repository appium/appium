...
import requestWithoutPromises from 'request';
...
async function downloadFile (fileUrl, suffix) {
  ...
  // we use the non-promise version of request because the 'pipe' function
  // won't overuse memory. request-promise has to store the whole response
  // in memory (and also has encoding trickiness)
  let writeStream = _fs.createWriteStream(fileInfo.path);
  writeStream.on('error', (e) => logger.errorAndThrow(e));

  let onClose = B.promisify(writeStream.on, writeStream)('close');

  requestWithoutPromises.get(fileUrl).pipe(writeStream);
