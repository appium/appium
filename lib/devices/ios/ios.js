...
IOS.prototype.findInPath = function (toFind, inPath, cb) {
  var findCmd = 'find ' + inPath + ' -name "' + toFind + '"';
...
IOS.prototype.getSimulatorApplications = function (cb) {
  ...
  var inPath = "/Users/" + user + "/Library/Application\\ " +
    "Support/iPhone\\ Simulator";
  var toFind = "Applications";

  this.findInPath(toFind, inPath, function (err, files) {
    if (err) {
      logger.error("Could not find: " + err.message);
      cb(err);
    }
    else {
      cb(null, files);
...
IOS.prototype.getSimulatorInstalledAppData = function (inPath, cb) {
  var toFind = "Documents";

  this.findInPath(toFind, inPath, function (err, dbfiles) {
    if (err) {
      logger.error("Could not find: " + err.message);
      cb(err);
    }
...
//            this.getSimulatorInstalledAppDBs(root, function (err) {
//              if (err) {
//                logger.error("Could not remove: " + err.message);
//                cb(err);
//              } else {
...
//                    rimraf.sync(dbfiles[i]);
//                    logger.info("Deleted " + dbfiles[i]);
//                  }
//                }
//              }
//            }.bind(this));
