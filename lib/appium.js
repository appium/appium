...
class AppiumDriver extends BaseDriver {
  ...
  pluginsToWrapCmd (cmd) {
    return this.plugins.filter((plugin) => plugin.wrapCommands.includes(cmd));
  }

  pluginsToHandleCmd (cmd) {
    return this.plugins.filter((plugin) => {
      if (plugin.commands === true) {
        return true;
      }
      if (_.isArray(plugin.commands) && plugin.commands.includes(cmd)) {
        return true;
      }
      return false;
    });
  ...
  async executeCommand (cmd, ...args) {
    // first things first, let's create a result object which we'll ultimately return.
    let res = {};

    // now we have basically three cases for how to handle commands:
    ...
    if (isSessionCmd) {
      ...
      // now reset the response protocol given that the session driver's protocol might differ
      res.protocol = dstSession.protocol;
    }
    ...
    // then we get any plugins which are registered as handling this command
    ...
    let cmdRes, cmdErr;
    try {
      // ok now we're done with all the plugin wrapping. At this point, `next` defines a whole
      // sequence of plugin handlers, culminating in our default handler. Whatever it returns is
      // what we're going to want to send back to the user.
      cmdRes = await next();
    } catch (e) {
      cmdErr = e;
    }
