...
Thanks for your contribution to Appium!Here are the principles we use when writing javascript. Please conform to these so we can merge your pull request
...
> 第二，如果我们需要变更你的代码，比起深入阅读源码中的执行步骤，看到一个逻辑变更会使我们的工作更加轻松，特别是如果你像我一样工作，让两三个工作并行进行。

如果有多位作者，请确保他们各自都有自己的提交记录，修改作者信息不是一个好主意。合并（merge）提交必须从 pull 请求中 rebase 。
...
not-yet-standard features, namely `async/await` (link needed). This style guide must be followed diligently in all Appium contributions!

我们使用未来版本的 JavaScript，并利用Babel转换器将其渲染到支持当前版本的 Node.js。我们使用 [ES2015](https://babeljs.io/)（以前称为ES6）中的一些非标准特性，如[`async / await`](http://babeljs.io/docs/plugins/syntax-async-functions/)。在所有Appium的贡献中必须严格遵循这些编码风格！
...
*   在列表（lists）,对象（objects）,函数调用（function calls）等代码块中，逗号和冒号后面需要添加一个空格：
