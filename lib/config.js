...
function getNonDefaultArgs (parser, args) {
  const nonDefaults = {};
  for (const [, {dest, default: defaultValue}] of parser.rawArgs) {
    if (args[dest] && args[dest] !== defaultValue) {
      nonDefaults[dest] = args[dest];
    }
  }
  return nonDefaults;
}
...
function getDeprecatedArgs (parser, args) {
  ...
  const deprecated = {};
  for (const [[name], {dest, default: defaultValue, action}] of parser.rawArgs) {
    if (!args[dest] || args[dest] === defaultValue) {
      continue;
    }

    if (action?.deprecated_for) {
      deprecated[name] = action.deprecated_for;
    } else if (_.includes(action?.name, 'DeprecatedDefaultCapability')) {
      deprecated[name] = '--defaultCapabilities';
    }
  }
  return deprecated;
}
