...
IOS.prototype.setPreferences = function (cb) {
  ...
  var setPrefs = function (err) {
    ...
    try {
      this.setAcceptCookiePrefs();
    } catch (e) {
      logger.error("Error setting NSHTTPAcceptCookies preferences, prefs will not work");
      logger.error(e);
      logger.error(e.stack);
    }
    cb();
...
IOS.prototype.setAcceptCookiePrefs = function () {
  var val;
  var cookieSettings = {};
  if (typeof this.capabilities.acceptCookies !== "undefined") {
    val = this.capabilities.acceptCookies;
    logger.debug("Setting NSHTTPAcceptCookies to " + val);
    cookieSettings.NSHTTPAcceptCookies = val;
    settings.updateSettings(this.sim, "webFoundation", cookieSettings);
  }
};
