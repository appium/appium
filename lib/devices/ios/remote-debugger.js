...
RemoteDebugger.prototype.selectApp = function (appIdKey, currentUrl, cb, maxTries, tries) {
  ...
  var onConnect = function (appIdKey, pageDict) {
    ...
    if (_.isEmpty(pageDict)) {
      return onRequiresRetry(this.appIdKey, "Empty page dictionary received. Trying again.");
    }

    if (currentUrl) {
      var found = false;
      _.each(pageDict, function (record) {
      ...
      if (!found) {
        return onRequiresRetry(this.appIdKey, "Received app, but url is not correct. Trying again.");
      }
    }
  ...
  var onRequiresRetry = function (correctAppIdKey, msg) {
    ...
    if (tries < maxTries) {
      ...
      this.selectApp(correctAppIdKey, currentUrl, cb, maxTries, tries + 1);
    } else {
