...
const {validator, CLIENT_URL_TYPES} = require('./validator');
...
async function registerSpecUrlHelper () {
  /**
   * ToDo(Christian):
   *  - direct to base driver in packages once ported
   *  - remove dependency from root package.json
   */
  const routesFile = await fs.readFile(path.resolve(ROOT_FOLDER, 'packages', 'appium', 'node_modules', 'appium-base-driver', 'lib', 'protocol', 'routes.js'), 'utf8');
  ...
  Handlebars.registerHelper('spec_url', function specUrl (specUrl, endpoint) {
    ...
    if (specUrl.startsWith('routes.js')) {
      /**
       * ToDo(Christian): update path to base-driver in this repo
       */
...
async function generateCommands () {
  ...
  const commands = path.resolve(ROOT_FOLDER, 'commands-yml', 'commands/**/*.yml');
  log('Traversing YML files', commands);
  await rimraf(path.resolve(ROOT_FOLDER, 'docs', 'en', 'commands'));

  // get the template from which the md files will be created
  const template = Handlebars.compile(await fs.readFile(path.resolve(__dirname, 'templates', 'template.md'), 'utf8'), {noEscape: true, strict: true});

  let fileCount = 0;
  for (const filename of await fs.glob(commands)) {
    const relativeFilename = path.relative(path.resolve(ROOT_FOLDER, 'commands-yml'), filename);
    ...
    inputJSON.ymlFileName = `/${path.relative(ROOT_FOLDER, filename)}`;
    ...
    const outfile = path.resolve(ROOT_FOLDER, 'docs', 'en', markdownPath);
...
async function generateCommandIndex () {
  ...
  const toc = require(path.resolve(ROOT_FOLDER, 'docs', 'toc.js'));
  ...
  const commandTemplate = Handlebars.compile(await fs.readFile(path.resolve(__dirname, 'templates', 'api-template.md'), 'utf8'), {noEscape: true, strict: true});
  ...
  const apiIndex = path.resolve(ROOT_FOLDER, 'docs', 'en', 'about-appium', 'api.md');
  ...
  async function writeIndividualIndexes (command) {
    ...
    const index = path.resolve(ROOT_FOLDER, relPath, 'README.md');
  ...
  const index = path.resolve(ROOT_FOLDER, 'docs', 'en', 'commands', 'README.md');
...
main().catch(
  (err) => {
    console.error(`Couldn't generate docs: ${err.stack}`);
    process.exit(1);
  }
);
