...
var Device = require('../device.js')
  , MockAppModel = require('./mock-app-model.js')
...
var MockDevice = function () {
  logger.debug("Instantiating MockDevice");
  this.appModel = null;
...
_.each(commonExtensions, function (ext) {
  MockDevice.prototype[ext] = Device.prototype[ext];
});
MockDevice.prototype._deviceConfigure = Device.prototype.configure;

MockDevice.prototype.configure = function (args, caps, cb) {
  ...
  this.appModel = new MockAppModel();
  this.appModel.loadApp(this.args.app, cb);
};

MockDevice.prototype.start = function (cb) {
  logger.debug("Starting MockDevice");
  cb();
};

MockDevice.prototype.stop = function (cb) {
  logger.debug("Stopping MockDevice");
  cb();
};

MockDevice.prototype.getCurrentContext = function (cb) {
  jwpSuccess(this.curContext, cb);
};

MockDevice.prototype._getRawContexts = function () {
...
MockDevice.prototype.getContexts = function (cb) {
  jwpSuccess(this._getRawContexts(), cb);
};

MockDevice.prototype.setContext = function (context, cb) {
...
MockDevice.prototype.implicitWait = function (ms, cb) {
  try {
