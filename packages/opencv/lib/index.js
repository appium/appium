...
async function getImagesMatches(img1Data, img2Data, options = {}) {
  ...
  try {
    ...
    if (!_.includes(_.keys(AVAILABLE_DETECTORS), detectorName)) {
      throw new Error(
        `'${detectorName}' detector is unknown. ` +
          `Only ${JSON.stringify(
            _.keys(AVAILABLE_DETECTORS)
          )} detectors are supported.`
      );
    }
    ...
    if (visualize) {
      ...
      result.visualization = await jimpImgFromCvMat(
        visualization
  ...
  try {
    ...
    if (visualize) {
      ...
      try {
        ...
        result.visualization = await jimpImgFromCvMat(resultMat).getBufferAsync(
          Jimp.MIME_PNG
        );
      } finally {
        try {
  ...
  try {
    ...
    try {
      ...
      if (multiple) {
        ...
        const {x, y} = method.includes('SQDIFF')
          ? minMax.minLoc
          : minMax.maxLoc;
        results.push({
          score: minMax.maxVal,
      ...
      if (_.isEmpty(results)) {
        ...
        throw new Error(
          `Match threshold: ${threshold}. Highest match value ` +
    ...
    if (visualize) {
      ...
      for (const result of results) {
        ...
        result.visualization = await jimpImgFromCvMat(
          singleHighlightedImage
        ).getBufferAsync(Jimp.MIME_PNG);
      }
      visualization = await jimpImgFromCvMat(
        fullHighlightedImage
      ).getBufferAsync(Jimp.MIME_PNG);
    }
    return {
      rect: results[0].rect,
...
function filterNearMatches(nonZeroMatchResults, matchNeighbourThreshold) {
  return nonZeroMatchResults.reduce((acc, element) => {
    if (
      !acc.some((match) => distance(match, element) <= matchNeighbourThreshold)
    ) {
      acc.push(element);
    }
    return acc;
