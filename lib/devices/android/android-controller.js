...
androidController.findUIElementOrElements = function (strategy, selector, many, context, cb) {
  ...
  if ("xpathCompression" in this.settings) {
    params.xpathCompression = this.settings.xpathCompression;
  }
...
androidController.isIMEActivated = function (cb) {
...
androidController.availableIMEEngines = function (cb) {
...
androidController.getActiveIMEEngine = function (cb) {
...
androidController.deactivateIMEEngine = function (cb) {
  logger.debug('Retrieving current default IME');
  this.adb.defaultIME(function (err, engine) {
    if (err) return cb(err);
