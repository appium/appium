...
function fixArgvForParser () {
  // this function mutates argv to ensure that argparse gets the 'server'
  // subcommand in the case where the user provides no subcommand
  if (process.argv.length < 2) {
    process.argv.push('server');
    return;
  }

  if (!_.includes(['driver', 'server', '-h'], process.argv[2])) {
