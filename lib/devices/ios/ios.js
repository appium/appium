    ...
    helpers.getiOSSDKVersion(function(err, sdkVersion) {
      ...
      if (sdkVersion !== null) {
        ...
        if (fs.existsSync(iosSimLocalePath)) {
          var foundSDK = false;
          for(var i=0; i < 10; i++) {
            var pathToCheck = path.resolve(process.env.HOME, "Library", "Application Support",
              "iPhone Simulator", sdkVersion + (i > 0 ? "." + i.toString() : ""),
              "Library", "Preferences", ".GlobalPreferences.plist");
            if (fs.existsSync(pathToCheck)) {
              foundSDK = true;
              sdkVersion += i > 0 ? "." + i.toString() : "";
              break;
            }
          }
          if (foundSDK) {
            ...
            msg = "Could not set locale information because the iOS Simulator's .GlobalPreferences.plist" +
