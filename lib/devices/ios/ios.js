...
IOS.prototype.init = function () {
  ...
  this.automationTraceTemplatePath = null;
  this.logs = {};
...
IOS.prototype.configure = function (args, caps, cb) {
  ...
  if (!this.args.automationTraceTemplatePath) {
    this.automationTraceTemplatePath = xcode.getAutomationTraceTemplatePath();
  } else {
    this.automationTraceTemplatePath = new Q(this.args.automationTraceTemplatePath);
  }
...
IOS.prototype.makeInstruments = function (cb) {
  ...
  Q.all([bootstrap, this.automationTraceTemplatePath])
  .spread(
    function (bootstrapPath, automationTraceTemplatePath) {
      ...
      , template: automationTraceTemplatePath
...
IOS.prototype.prelaunchSimulator = function (cb) {
  ...
  xcode.getPath(function (err, xcodePath) {
    ...
    xcodeFolder = xcodePath;
  });

  logger.debug("Pre-launching simulator");
  var iosSimPath = path.resolve(xcodeFolder,
