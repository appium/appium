...
RemoteDebugger.prototype.selectApp = function (appIdKey, cb) {
  ...
  this.send(connectToApp, function (appIdKey, pageDict) {
...
RemoteDebugger.prototype.setHandlers = function () {
  this.handlers = {
    ...
    '_rpc_applicationConnected:': function (plist) {
      this.logger.info("We were notified that we connected to possibly " +
                       "the wrong application. Ignoring for now and " +
                       "hoping we're going to retry looking for apps");
      this.logger.debug(JSON.stringify(plist.__argument));
    }.bind(this),
