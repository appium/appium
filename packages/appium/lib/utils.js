...
function parseCapsForInnerDriver(
  ...
  const hasW3CCaps =
    ...
    (_.has(w3cCapabilities, 'alwaysMatch') ||
      _.has(w3cCapabilities, 'firstMatch'));
  ...
  if (!_.isEmpty(defaultCapabilities)) {
    if (hasW3CCaps) {
      for (const [defaultCapKey, defaultCapValue] of _.toPairs(
        defaultCapabilities
      )) {
        ...
        for (const firstMatchEntry of w3cCapabilities.firstMatch || []) {
          if (
            ...
            _.has(
              removeAppiumPrefixes(firstMatchEntry),
