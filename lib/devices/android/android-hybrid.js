...
androidHybrid.setupExistingChromedriver = function (context, cb) {
  ...
  new Q(this.chromedriver.hasWorkingWebview()).then(function (works) {
    if (works) return;
    ...
    return new Q(this.chromedriver.restart()).then(function () {
...
androidHybrid.stopChromedriverProxies = function (ocb) {
  async.eachSeries(Object.keys(this.sessionChromedrivers), function (context, cb) {
    ...
    new Q(this.sessionChromedrivers[context].stop())
      .catch(function (err) { logger.warn("Error stopping Chromedriver: " + err.message); })
      .finally(function () { delete this.sessionChromedrivers[context]; }.bind(this))
      .nodeify(cb);
