...
async function registerToGrid (options_post, jsonObject) {
  try {
    ...
    let logMessage = `Appium successfully registered with the grid on ${jsonObject.hubHost}:${jsonObject.hubPort}`;
...
async function postRequest (data, addr, port) {
  ...
  // backward compatibility with Selenium 2 config files
  if (jsonObject.hasOwnProperty('configuration')) {
    for (var property in jsonObject.configuration) {
      if (jsonObject.configuration.hasOwnProperty(property)) {
        jsonObject[property] = jsonObject.configuration[property];
      }
    }
  }
  ...
  if (!jsonObject.url || !jsonObject.host || !jsonObject.port) {
    jsonObject.url = `http://${addr}:${port}/wd/hub`;
    jsonObject.host = addr;
    jsonObject.port = port;
  }
  // if the node config does not have id automatically add it
  if (!jsonObject.id) {
    jsonObject.id = `http://${jsonObject.host}:${jsonObject.port}`;
  }
  ...
  // the post options
  let post_options = {
    url: `http://${jsonObject.hubHost}:${jsonObject.hubPort}/grid/register`,
    method: 'POST',
    body: data,
  ...
  if (jsonObject.register !== true) {
    logger.debug(`No registration sent (${jsonObject.register} = false)`);
    return;
  }

  let registerCycleTime = jsonObject.registerCycle;
...
async function isAlreadyRegistered (jsonObject) {
  ...
  try {
    let response = await request({
      uri: `http://${jsonObject.hubHost}:${jsonObject.hubPort}/grid/api/proxy?id=${id}`,
      method  : 'GET',
      timeout : 10000,
      resolveWithFullResponse: true // return the full response, not just the body
    });
