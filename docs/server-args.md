...
|`-r`, `--remove`|false|(IOS-only) Remove Instruments trace directories||
|`-s`, `--reset`|false|Reset app state after each session (IOS: delete plist; Android: install app before session and uninstall after session)||
