...
validate.validators.hasPossibleAttributes = function hasPossibleAttributes(
  value,
  options
) {
  if (!value) {
...
Handlebars.registerHelper(
  'versions',
  function versionHelper(object, name, driverName) {
    if (!object) {
      return 'None';
    }
...
Handlebars.registerHelper('client_url', function clientUrl(clientUrl) {
  ...
  const createUrlString = function createUrlString(
    clientUrl,
    name = getBaseHostname(clientUrl)
  ) {
...
async function generateCommands() {
  ...
  for (const filename of await fs.glob(commands)) {
    ...
    if (validationErrors) {
      throw new Error(
        `Data validation error for ${filename}: ${JSON.stringify(
          validationErrors
        )}`
      );
    }
...
async function generateCommandIndex() {
  ...
  async function writeIndividualIndexes(command) {
    ...
    // write this node
    const relPath = command.path.startsWith(path.sep)
