...
|`customSnapshotTimeout` (`snapshotTimeout` before 1.19.1) | Set how much time is allowed to resolve a single accessibility snapshot with custom attributes. _Snapshots_ are mainly used for page source generation, XML lookup and custom attributes retrieval (these are visibility and accessibility ones). It might be necessary to increase this value if the actual page source is very large and contains hundreds of UI elements. Defaults to 15 seconds. | e.g. `10`, `100` (seconds) |1.15.0+|
|`waitForIdleTimeout`|The amount of time in float seconds to wait until the application under test is idling. XCTest requires the app's main thread to be idling in order to execute any actions on it, so each action, like single click, might take a lot of time in case your app is constantly hogging the main thread. The default value is `10`. Setting it to zero disables idling checks completely (not recommended).|e.g. `2.5`|1.19.1|
|`waitForAnimationTimeout`|The amount of time in float seconds to wait until the application under test does not have any active animations. This check is usually applied after each action, like  `click` one and is started in parallel with the idling check (see `waitForIdleTimeout` setting). The default value is `2`. Setting it to zero disables animation checks completely.|e.g. `1.5`|1.19.1|
