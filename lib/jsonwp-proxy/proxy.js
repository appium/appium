...
class JWProxy {
  ...
  async proxy (url, method, body = null) {
    ...
    log.info(`Proxying [${method} ${url || "/"}] to [${method} ${newUrl}] ` +
             (body ? `with body: ${_.truncate(JSON.stringify(body), LOG_LENGTH)}` : 'with no body'));
    let res, resBody;
    try {
      res = await this.request(reqOpts);
      resBody = res.body;
      log.info(`Got response with status ${res.statusCode}: ${_.truncate(JSON.stringify(resBody), LOG_LENGTH)}`);
