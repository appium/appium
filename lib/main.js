...
async function main (args = null) {
  ...
  if (args.logFilters) {
    ...
    if (!_.isEmpty(issues)) {
      logger.warn(`The log filtering rules config '${args.logFilters}' has issues: ` +
        JSON.stringify(issues, null, 2));
    }
    if (_.isEmpty(rules)) {
      logger.info(`Loaded no log filtering rules from '${args.logFilters}'`);
    } else {
      logger.info(`Loaded ${util.pluralize(rules, 'filtering rule', true)} from '${args.logFilters}'`);
    }
  }
