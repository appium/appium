...
| mobile:flashElement | Flashes an element on the screen to visually confirm you are operating on the correct element. Can optionally set `duration` (in ms) and `repeatCount` of animation| `{elementId, duration, repeatCount}` | `{elementId: "1234-abcd-5678", duration: 1000, repeatCount: 10}` |
