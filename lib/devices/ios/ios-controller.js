...
iOSController.createGetElementCommand = function (strategy, selector, ctx,
  ...
  case "id":
    ...
    command = ["var exact = au.mainApp().getFirstWithPredicateWeighted",
               "(\"name == '", selector, "' || label == '", selector,
               "' || value == '", selector, "'\");"].join('');
    command += ["exact && exact.status == 0 ? ",
                "exact : au.mainApp().getFirstWith",
                "PredicateWeighted(\"name contains[c] '", selector,
                "' || label contains[c] '", selector,
                "' || value contains[c] '", selector, "'\");"].join('');
    break;
