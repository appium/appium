...
async function runExtensionCommand (args, type) {
  ...
  const config = type === DRIVER_TYPE ? new DriverConfig(appiumHome, logFn) : new PluginConfig(appiumPluginHome || appiumHome, logFn); const CommandClass = type === DRIVER_TYPE ? DriverCommand : PluginCommand;
