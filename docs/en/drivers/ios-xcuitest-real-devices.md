...
1. Consider to generate provisioning profile with `.xctrunner` identifier if you would not like to generate a wildcard provisioning profile for manual configuration. The `.xctrunner` is added since Xcode 11. [A reference](https://github.com/appium/appium/issues/13610)
