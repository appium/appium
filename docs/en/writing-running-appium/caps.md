...
|`remoteAppsCacheLimit`| Set the maximum number of remote cached apks (default is 10) which are pushed to the device's under test local storage. Caching of remote apks speeds up the execution of sequential test scenarios for the same set of apks by avoiding them to be pushed to the remote file system each time reinstall is needed. Set this capability to `0` in order to disable caching. | e.g. `0`, `5`, `20` |
