...
WebKitRemoteDebugger.prototype.connect = function (pageId, cb, pageChangeCb) {
  ...
  this.socket.on('error', function (exception) {
    this.logger.debug('Debugger web socket error: ' + exception.message);
  }.bind(this));
  this.socket.on('message', function (data) {
    var truncated = data || '';
    if (truncated.length > 300) {
      truncated = truncated.slice(0, 300) + '...';
    }
    this.logger.debug('Debugger web socket received data: ' + truncated);
    this.receive(data);
  }.bind(this));
};
...
WebKitRemoteDebugger.prototype.send = function (data, cb) {
  ...
  //send the data
  var truncated = JSON.stringify(data).slice(0, 200) + "...";
  this.logger.debug('Remote debugger data sent [' + truncated + ']');
