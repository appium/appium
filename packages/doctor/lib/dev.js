...
class BinaryIsInPathCheck extends DoctorCheck {
  ...
  fix() {
    return `Manually install the ${this.binary.bold} binary and add it to ${
      'PATH'.bold
    }.`;
  }
}
...
class AndroidSdkExists extends DoctorCheck {
  ...
  async diagnose() {
    if (typeof process.env.ANDROID_HOME === 'undefined') {
      return nok(
        `${this.sdk} could not be found because ANDROID_HOME is NOT set!`
      );
    }
    let sdkPath = path.resolve(
      process.env.ANDROID_HOME,
      path.join('platforms', this.sdk)
    );
