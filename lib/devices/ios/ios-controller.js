...
var _setContext = function (name, cb, skipReadyCheck) {
...
iOSController.setContext = function (name, cb, skipReadyCheck) {
  var ctx = this.getCachedContext(name);
  if (ctx) {
    this.curContext = ctx;
    return cb(null, {
      status: status.codes.Success.code
  ...
  _setContext.call(this, name, function (err, res) {
    if (!err && res.status === status.codes.Success.code) {
      this.cacheContext(res.name, this.curContext);
    }
    cb(err, res);
  }.bind(this), skipReadyCheck);
};
