...
function updateBuildInfo () {
  return new B((resolve) => {
    getGitRev().then((sha) => { // eslint-disable-line promise/prefer-await-to-then
      if (_.isEmpty(sha)) {
        logger.warn(`Cannot retrieve git revision`);
        return resolve();
      }

      BUILD_INFO['git-sha'] = sha;
      getGitTimestamp(sha)
        .then((built) => { // eslint-disable-line promise/prefer-await-to-then
          if (!_.isEmpty(built)) {
            BUILD_INFO.built = built;
          } else {
            logger.warn(`Cannot retrieve build timestamp`);
          }
          resolve();
        }).catch((e) => {
          logger.warn(`Cannot retrieve build timestamp. Original error: ${e.message}`);
          resolve();
        });
    }).catch((e) => {
      logger.warn(`Cannot retrieve git revision. Original error: ${e.message}`);
      resolve();
    });
  });
}
...
async function getGitTimestamp (commitSha) {
  if (await fs.exists(path.resolve(rootDir, GIT_META_ROOT))) {
