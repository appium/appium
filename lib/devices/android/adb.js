    ...
    if (err || emulators.length < 1) {
      ...
      async.forEach(emulators, function (emulator, asyncCb) {
        ...
        if (!emulator.genymotion) {
          ...
          this.getGenymotionAvdName(this.getGenymotionIp(emulator), function (err, runningAVDName) {
            if (avdName === runningAVDName) {
...
ADB.prototype.launchAVD = function (avdName, avdArgs, genymotion, cb) {
  ...
  if (!genymotion) {
    ...
    // genymotion launch
    this.checkGenymotionPlayerPresent(function (err, gmPlayer) {
      if (err) return cb(err);
      var gmArgs = ["--vm-name", avdName];
      this.debug("GMLaunch:" + gmPlayer);
...
ADB.prototype.getConnectedEmulators = function (cb) {
  ...
  this.getConnectedDevices(function (err, devices) {
    ...
    _.each(devices, function (device) {
      ...
      if (port) {
        device.port = port;
        device.genymotion = this.isEmulatorGenymotion(device.udid);
...
ADB.prototype.isScreenLocked = function (cb) {
  ...
  this.shell(cmd, function (err, stdout) {
    ...
    try {
      ...
      logger.info("Could not write the .dumpsys.log file");
    }
...
ADB.prototype.getGenymotionAvdName = function (ip, cb) {
  logger.info("Finding avdname from genymotion ip: " + ip);
...
ADB.prototype.isEmulatorGenymotion = function (udid) {
  return udid.indexOf(":") !== -1;
};

ADB.prototype.getGenymotionPath = function () {
  var gm;
  if (isMac) {
  ...
  this.debug("Genymotion player:" + gm);
  return gm;
};

ADB.prototype.checkGenymotionPlayerPresent = function (cb) {
  var gmPath = this.getGenymotionPath();
  if (fs.existsSync(gmPath)) {
    cb(null, gmPath);
  } else {
    cb(new Error("Could not find Genymotion."));
  }
};

ADB.prototype.getVBoxmanagePath = function (cb) {
  var vbm;
  if (isWindows) {
    vbm = process.env.ProgramFiles + "\\Oracle\\VirtualBox\\VBoxManage.exe";
    cb(null, vbm);
  } else {
    exec("which vboxmanage", function (err, stdout) {
      if (stdout) {
        stdout = stdout.trim();
      }
      cb(err, stdout);
...
ADB.prototype.checkVBoxManagePresent = function (cb) {
...
ADB.prototype.getGenymotionIp = function (emulator) {
  var ip = null;
