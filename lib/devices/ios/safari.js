...
Safari.prototype._clickButtonToLaunchSafari = function (cb) {
  this.findElement('accessibility id', 'launch safari',
                   function (err, res) {
    if (err || res.status !== 0) {
      var msg = "Error. Could not find button to launch Safari. " +
    ...
    var buttonId = res.value.ELEMENT;

    this.click(buttonId, function (err, res) {
      if (err || res.status !== 0) {
        var msg = "Error. For whatever reason, could not click " +
                  "the 'launch safari' button of SafariLauncher";
        logger.error(msg);
        return cb(new Error(msg));
      }
      ...
      // we're done with instruments at this point, we go on to Safari
...
Safari.prototype.start = function (cb, onDie) {
  ...
  this._start(function (err) {
    ...
    if (this.args.udid) {
      logger.debug("On Safari Launcher. Tapping button to launch Safari");

      this._clickButtonToLaunchSafari(function (err) {
        if (err) return cb(err);
