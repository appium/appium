...
iOSController.findUIElementsByXpath = function (selector, ctx, many, curRetry,
  ...
  this.getSourceForElementForXML(ctx, function (err, res) {
    ...
    if (!many) {
      this.proxy("au.getElementByIndexPath('" + indexPaths[0] + "')", cb);
    } else {
      // having multiple index paths means we think multiple elements
      // should be there. Sometimes uiauto lags in enabling us to get elements,
      // so we retry a few times if it can't find elements we know should be
      // there. see uiauto code for more logic
      var ipArrString = JSON.stringify(indexPaths);
      var proxyCmd = "au.getElementsByIndexPaths(" + ipArrString + ")";
      this.proxy(proxyCmd, function (err, res) {
        ...
        if (res.status === status.codes.StaleElementReference.code &&
            curRetry < 3) {
