...
| `screenshotOrientation` | Adjust screenshot orientation for iOS. Appium tries to return a screenshot correctly adjusting the orientation automatically by default, but it sometimes does not work, especially landscape mode. It depends on various factors such as OS versions, model versions and real or simulator devices. This option allows you to adjust the coordinate. Defaults to `auto`.| `auto`, `portrait`, `portraitUpsideDown`, `landscapeRight` | 1.17.0+ |
