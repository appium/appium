...
|`appLocale`| Set [Locale](https://developer.android.com/reference/java/util/Locale) for [the target context](https://developer.android.com/reference/androidx/test/core/app/ApplicationProvider#getApplicationContext()) since Appium 1.18.0. `language`, `country` and `variant` are available as same as the Locale class. `language` is mandatory. Please check locale page for more details. The appLocale affects in application wide, so this does not work for system such as status bar. Please refer to `language`, `locale` and `localeScript` in general capabilities when you want to change locale entire the system. | e.g., `{"language": "ja", "country": "JP"}` |
