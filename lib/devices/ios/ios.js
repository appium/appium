  ...
  if (this.shouldIgnoreInstrumentsExit()) {
    ...
    this.instruments.start(
      function (err) {
        ...
        // Instruments will never check into the command proxy, so we'll never
        // get here except in the error case; that's why we don't call cb()
        // here
      }
    , function () {
        // Instead, we'll be notified that instruments unexpectedly exited.
        // This is actually not unexpected in this case, and we take it
        // as a sign that instruments has launched SafariLauncher and Safari
        // is now ready to go.
        cb();
      }
    );
