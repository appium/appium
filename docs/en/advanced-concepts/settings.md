...
| `includeNonModalElements` | Whether returns all of elements including no modal dialogs on iOS 13+. It fixes [cannot find elements on nested modal presentations](https://github.com/appium/appium/issues/13227), but it might make visibility attributes unreliable. Then, you can also enable `shouldUseTestManagerForVisibilityDetection` settings (defaults to `false`) or `simpleIsVisibleCheck` capability to improve the visibility detection. Defaults to `false`. | `true`, `false` |1.15.0+|
| `acceptAlertButtonSelector` | Allow to customize accept alert button selector. It helps you to handle an arbitrary element as accept button in `accept alert` command. It supports _class chain_ locator. Defaults to find a button as class type of `XCUIElementTypeAlert`. | <code>**/XCUIElementTypeButton[`label CONTAINS[c] 'accept'`]</code> | 1.16.0+ |
| `dismissAlertButtonSelector` | Allow to customize dismiss alert button selector. It helps you to handle an arbitrary element as dismiss button in `dismiss alert` command. It supports _class chain_ locator. Defaults to find a button as class type of `XCUIElementTypeAlert`. | <code>**/XCUIElementTypeButton[`label CONTAINS[c] 'dismiss'`]</code> | 1.16.0+ |
