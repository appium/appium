...
IosLog.prototype.startCapture = function (cb) {
  ...
  if (this.udid) {
    ...
    which("idevicesyslog", function (err, pathToCmd) {
      logger.warn('err', err, 'path', pathToCmd);
      if (!err) {
...
IosLog.prototype.finishStartingLogCapture = function (cb) {
  if (!this.proc) {
    logger.warn("Could not capture device log");
    return cb(new Error("could not capture device log"));
  }
