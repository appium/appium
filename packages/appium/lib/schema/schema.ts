...
class AppiumSchema {
  ...
  async finalize(): Promise<Readonly<Record<string, StrictSchemaObject>>> {
    ...
    for (
      const [extType, extensionSchemas] of Object.entries(this.#registeredSchemas) as Array<
    ...
    super(
      (() => {
        ...
        if (util.isPlainObject(schema)) {
          ...
          throw new TypeError(
            `schema IS supported; this error should not be thrown (this is a bug). value of schema: ${
              JSON.stringify(
                schema,
              )
            }`,
