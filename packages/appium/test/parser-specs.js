...
describe('Server Parser', function () {
  ...
  describe('extension arguments', function () {
    ...
    // XXX: since these tests are not isolated and operate on the _actual_ drivers/plugins,
    // and we do not have a plugin which accepts these arguments, we cannot execute this.
    it.skip('should parse default plugin args correctly from a string', function () {
      let fakePluginArgs = {fake: {sillyWebServerPort: 1234, sillyWebServerHost: 'hey'}};
      const args = p.parse_args([
        '--plugin-fake-silly-web-server-port',
        fakePluginArgs.fake.sillyWebServerPort,
        '--plugin-fake-silly-web-server-host',
        fakePluginArgs.fake.sillyWebServerHost
      ]);
      args.plugin.fake.should.eql(fakePluginArgs.fake);
    });
  });
});
