    ...
    withMocks({fs}, (mocks) => {
      ...
      it('failure - missing android home', async function () {
        ...
        (await check.diagnose()).should.deep.equal({
          ...
          optional: false,
          message:
            'android-16 could not be found because ANDROID_HOME is NOT set!',
        });
        mocks.verify();
      });
      it('diagnose - failure - invalid path', async function () {
        ...
        (await check.diagnose()).should.deep.equal({
          ...
          optional: false,
          message:
            "android-16 could NOT be found at '/a/b/c/d/platforms/android-16'!",
        });
        mocks.verify();
      });
      ...
      it('fix - install', async function () {
        process.env.ANDROID_HOME = '/a/b/c/d';
        removeColors(await check.fix()).should.equal(
          'Manually install the android-16 sdk.'
        );
      });
    })
  );
