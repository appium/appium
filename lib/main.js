...
async function main (args = null) {
  ...
  // turn all of the installed plugins into instantiated plugin class objects, so we can send them
  // as objects to the server init. we also want to send/assign them to the umbrella driver so it can
  // activate them on a per-request basis as well
  const plugins = Object.keys(pluginConfig.installedExtensions).map((plugin) => {
    const PluginClass = pluginConfig.require(plugin);
    return new PluginClass(plugin);
  });
