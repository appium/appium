...
Setup is not interrupted. With an accepted idempotency key, Appium keeps the original session
and caches its response. A retry with the same key waits for setup to finish and receives the
same session ID, or the same error if setup failed. This also works after the original client
disconnects, while the response remains in Appium's bounded, in-memory idempotency cache.
