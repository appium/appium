...
describe('errors', function () {
  for (let error of errorsList) {
    it(
      error.errorName + ' should have a JSONWP code or W3C code and message',
      function () {
        if (error.errorCode) {
          new errors[error.errorName]().should.have.property(
            'jsonwpCode',
            error.errorCode
          );
        } else {
...
describe('errorFromW3CJsonCode', function () {
    ...
    if (error.errorName !== 'NotYetImplementedError') {
      it(error.errorName + ' should return correct error', function () {
        ...
        if (w3cError) {
          ...
          errorFromW3CJsonCode(w3cError).should.have.property(
            'message',
            error.errorMsg
          );
        } else {
          isErrorType(errorFromW3CJsonCode(w3cError), errors.UnknownError)
            .should.be.true;
        }
      });
    }
  ...
  it('should parse unknown errors', function () {
    isErrorType(
      errorFromW3CJsonCode('not a real error code'),
      errors.UnknownError
    ).should.be.true;
    errorFromW3CJsonCode('not a real error code').message.should.match(
      /An unknown server-side error occurred/
    );
    errorFromW3CJsonCode('not a real error code').error.should.equal(
      'unknown error'
    );
  });
});
describe('w3c Status Codes', function () {
  it('should match the correct error codes', function () {
    ...
    for (let [errorName, expectedErrorCode] of non400Errors) {
      ...
      new errors[errorName]().should.have.property(
        'w3cStatus',
        expectedErrorCode
      );
    }
    ...
    new errors.ElementClickInterceptedError().should.have.property(
      'w3cStatus',
      400
    );
  });
...
describe('.getResponseForW3CError', function () {
  ...
  it('should return an error, message and stacktrace for a NoSuchElementError', function () {
    const noSuchElementError = new errors.NoSuchElementError(
      'specific error message'
    );
    const [httpStatus, httpResponseBody] =
      getResponseForW3CError(noSuchElementError);
  ...
  it('should handle BadParametersError', function () {
    const badParamsError = new errors.BadParametersError(
      '__FOO__',
      '__BAR__',
      '__HELLO_WORLD__'
    );
    const [httpStatus, httpResponseBody] =
      getResponseForW3CError(badParamsError);
...
describe('.getActualError', function () {
  describe('MJSONWP', function () {
    ...
    it('should map a status code 10, StaleElementReferenceError', function () {
      ...
      isErrorType(actualError, errors.StaleElementReferenceError).should.be
        .true;
    });
