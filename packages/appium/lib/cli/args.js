...
function makeRunArgs (type) {
  return [
    ...
    [[type], {
      type: 'str',
      help: `Name of the ${type} to install, for example: ` + DRIVER_EXAMPLE
    }],
    [['scriptName'], {
      default: null,
      type: 'str',
      help: `Name of the script to run against the driver. The script name must be cached inside the "scripts" field under "appium" inside the driver's "package.json" file`
    }],
  ];
}
