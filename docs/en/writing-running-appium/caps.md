...
|`skipServerInstallation`|Skip uiAutomator2 server installation and use uiAutomator2 server from the device. Can be used to improve startup performance when uiAutomator2 server in valid version is already installed on the device. Defaults to `false` | `true` or `false`|
