...
androidController.setLocation = function(latitude, longitude, altitude, horizontalAccuracy, verticalAccuracy, course, speed, cb) {
    var telnetCmd = "geo fix " + longitude + " " + latitude;
    var shellCmd = 'am startservice -e longitude ' + '"' + longitude + '"' + ' -e latitude ' + '"' + latitude + '"' + ' io.appium.settings/.LocationService';
    var deviceId = this.adb.curDeviceId;
    var portPattern = /emulator-(\d+)/;
    var isDevice = (portPattern.test(deviceId));

    if (isDevice) {
        this.adb.shell(shellCmd, function(err) {
            if (err) {
                logger.debug("Error occurred in the Device :" + err);
                return cb(null, {
                    status: status.codes.UnknownError.code,
                    value: "Could not set geolocation to the Device"
                });
            }
            cb(null, {
                status: status.codes.Success.code,
                value: "Location set successfully"
            });
        });
    } else {
        this.adb.sendTelnetCommand(telnetCmd, function(err, res) {
            if (err) {
                logger.debug("Error occurred in the Emulator :" + err);
                return cb(null, {
                    status: status.codes.UnknownError.code,
                    value: "Could not set geolocation to the Emulator"
                });
            }
            cb(null, {
                status: status.codes.Success.code,
