...
var Chromedriver = function (args, onDie) {
  ...
  this.remoteChromedriver = false;

  if (args.remoteChromedriverHost) {
    this.remoteChromedriver = true;
    this.proxyHost = args.remoteChromedriverHost;
    this.proxyPort = args.remoteChromedriverPort;
  }
...
Chromedriver.prototype.ensureChromedriverExists = function (cb) {
  if (this.remoteChromedriver) {
    logger.info("skipping chromedriver existance test since we're connecting to a remote server");
    return cb();
  }
...
Chromedriver.prototype.stop = function (cb) {
  if (this.remoteChromedriver) {
    logger.info("Not killing chromedriver since we have a remote server");
