...
function contextTests() {
  describe('contexts, webviews, frames', function () {
    ...
    it('should get current context', async function () {
      await driver.getContext().should.eventually.become('NATIVE_APP');
    });
    it('should get contexts', async function () {
      await driver.getContexts().should.eventually.become(['NATIVE_APP', 'PROXY', 'WEBVIEW_1']);
    });
    it('should not set context that is not there', async function () {
      await driver
        .switchContext('WEBVIEW_FOO')
    ...
    it('should find webview elements in a webview', async function () {
      await (await driver.$('//*')).getTagName().should.eventually.become('html');
    });
    ...
    it('should not set a frame in a native context', async function () {
      ...
      await driver
        .switchToFrame(1)
        .should.eventually.be.rejectedWith(/could not be executed in the current context/);
    });
  });
}
