...
deletes the new session once driver setup completes. Setup is not interrupted; cleanup runs
after it finishes. A retry with the same `X-Idempotency-Key` waits until the abandoned session
has been deleted, then starts a fresh session. Other retries with that key share the fresh
response. The replacement session stays active if its response is sent successfully.
