...
| `enableWebviewDetailsCollection` | Prints WebView detailed information via Chrome Debugger Protocol endpoint as the Appium log since Appium 1.18.0+. It helps to confirm the version of WebView the device under test has, for example. Defaults to `false` | `true` or `false` |
