...
async function getAppiumConfig () {
  let stat = await B.promisify(fs.stat)(path.resolve(__dirname, '..'));
...
function checkNodeOk () {
  ...
  if (major === 0 && minor < 10) {
    let msg = `Node version must be >= 0.10.`;
    throw new Error(msg);
  }
}
...
function checkValidPort (port) {
  ...
  logger.error("Port must be greater than 0 and less than 65536");
  return false;
}
