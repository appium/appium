...
exports.performTouch = function (req, res) {
  ...
  // in a WebView performing a gesture on an element isn't supported yet
  var elsUsed = _.filter(_.pluck(_.pluck(gestures, "options"), "element"),
                         function (i) { return !_.isUndefined(i); });
  if (req.device.isWebContext() && elsUsed.length > 0) {
