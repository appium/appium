...
var getPath = xcode.getPath = function (cb) {
...
xcode.getVersion = function (cb) {
  xcode.getPath(function (err, xcodePath) {
    ...
    if (!fs.existsSync(plistPath)) {
      ...
      var cmd;
      try {
        cmd = "/usr/libexec/PlistBuddy -c 'Print CFBundleShortVersionString' " +
...
xcode.getAutomationTraceTemplatePath = function () {
    logger.debug("Detecting automation tracetemplate");
    var deferred = Q.defer();
    var msg;

    getPath(function (err, xcodeFolderPath) {
      ...
      var pathPrefix = path.resolve(xcodeFolderPath, "../Applications/Instruments.app/Contents/PlugIns");
      ...
      if (fs.existsSync(automationTraceTemplatePaths[0])) {
        deferred.resolve(automationTraceTemplatePaths[0]);
      } else if (fs.existsSync(automationTraceTemplatePaths[1])) {
        deferred.resolve(automationTraceTemplatePaths[1]);
      } else {
        msg = "Could not find Automation.tracetemplate in any of the following locations " +
              automationTraceTemplatePaths.toString();
        logger.error(msg);
