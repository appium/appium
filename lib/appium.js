...
class AppiumDriver extends BaseDriver {
  ...
  async getDriverForCaps (caps) {
    ...
    if (_.isString(caps.automationName)) {
      for (const {automationName, driverClass} of _.values(driversMapping)) {
        if (!_.isString(automationName)) {
          continue;
        }
        if (caps.automationName.toLowerCase() === automationName.toLowerCase()) {
          return driverClass;
        }
      }
    }
  ...
  async getDriverVersion (driver) {
    const driversMapping = await this.getDriversMapping();
    for (const [driverName, driverProps] of driversMapping) {
      if (driverName === driver.name && driverProps.moduleInfo && driverProps.moduleInfo.version) {
        return driverProps.moduleInfo.version;
      }
    }
...
async function lookupModuleInfo (driverClass) {
  for (const [modPath, modInfo] of _.values(require.cache)) {
    for (const exportedValueName of _.keys(modInfo.exports)) {
      if (exportedValueName !== driverClass.name) {
        continue;
      }
