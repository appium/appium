...
var IosLog = function(opts) {
  ...
  this.logs = [];
  this.logRow = null;
  this.logPrefix = "[IOS_LOG_CAPTURE] ";
};
...
IosLog.prototype.startCapture = function(cb) {
  ...
  if (this.udid) {
    logger.info((this.logPrefix + "Starting iOS device log capture via idevicesyslog").grey);
    this.proc = spawn("idevicesyslog", ["-u", this.udid]);
  }
  else {
    if (parseInt(this.xcodeVersion.split(".")[0]) >= 5) {
      logger.info((this.logPrefix + "Starting iOS 7.* simulator log capture").grey);
      this.proc = spawn("tail", ["-f", process.env.HOME + "/Library/Logs/iOS Simulator/7.0/system.log"]);
    }
    else {
      logger.info((this.logPrefix + "Starting iOS 6.* simulator log capture").grey);
  ...
  this.proc.on('error', function(err) {
    logger.error((this.logPrefix + "iOS log capture failed: " + err.message).grey);
    if (!this.calledBack) {
...
IosLog.prototype.stopCapture = function() {
  logger.info((this.logPrefix + "Stopping iOS log capture").grey);
  this.proc.kill();
  this.proc = null;
};
...
IosLog.prototype.onStderr = function(data) {
  if (/execvp\(\)/.test(data)) {
    logger.error((this.logPrefix + "iOS log capture process failed to start").grey);
    if (!this.calledBack) {
      this.calledBack = true;
