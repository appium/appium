...
|`calendarAccessAuthorized`|If truthy, enables calendar access on IOS Simulator with given bundleId. If `false`, disables calendar access on IOS Simulator with given bundleId. By default the calendar authorizationStatus remains unchanged.|e.g., `true`|
