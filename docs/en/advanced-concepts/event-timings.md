...
You can send a custom event name to Appium server as [Log Event API](docs/commands/session/events/log-event), then Appium server store the time. [Get events](docs/commands/session/events/get-events) returns stored events.
