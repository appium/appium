...
function getNonDefaultArgs (parser, args) {
  ...
  for (let rawArg of parser.rawArgs) {
    ...
    if (args[arg] && args[arg] !== rawArg[1].default) {
...
function getDeprecatedArgs (parser, args) {
  ...
  const deprecated = {};
  for (const rawArg of parser.rawArgs) {
    const arg = rawArg[1].dest;
    const defaultValue = rawArg[1].default;
    const isDeprecated = _.includes(rawArg[1].help, 'DEPRECATED');
    const deprecatedArgNameMatch = /\b(--\w[\w-]+)\b/.exec(rawArg[1].help || '');
    if (args[arg] && args[arg] !== defaultValue && isDeprecated && deprecatedArgNameMatch) {
      deprecated[rawArg[0]] = deprecatedArgNameMatch[1];
    }
  }
  return deprecated;
}
