...
var IOS = function(args) {
  ...
  this.deviceSpec = args.deviceSpec;
  ...
  this.appiumForMacProcess = null;
  this.appiumForMacDriver = null;
  this.queue = [];
  this.progress = 0;
...
IOS.prototype.start = function(cb, onDie) {
  ...
  async.series([
    ...
    this.preLaunchSimulatorAndSetOptions.bind(this),
...
IOS.prototype.connectAppiumForMacDriver = function (cb) {
  var msg;
  if (this.appiumForMacProcess) {
    cb();
  } else {
    var appiumForMacPath = path.resolve(__dirname, "../../../build/appium-for-mac/Release/AppiumForMac.app/Contents/MacOS/AppiumForMac");
    if (fs.existsSync(appiumForMacPath)) {
      logger.info('Launching Appium for Mac to control the iOS Simulator');
      this.appiumForMacProcess = spawn(appiumForMacPath, []);
      if (!this.udid && this.deviceSpec) {
        if (this.appiumForMacDriver) {
          try {
            this.appiumForMacDriver.quit();
          } catch (err) {
            logger.warn('Could not quit Appium For Mac driver');
          }
        }
        this.appiumForMacDriver = wd.remote("localhost", 4622);
        this.appiumForMacDriver.on('status', function(info){
          logger.debug('Appium for Mac Client > STATUS:' + info);
        });
        this.appiumForMacDriver.on('command', function(meth, path, data){
          logger.debug('Appium for Mac Client > ' + meth + " " + path + ": " + data || '');
        });

        // TODO: add proper wait loop for appium for mac to boot
        setTimeout(function() {
          this.appiumForMacDriver.init({browserName:'mac'}, function() {
            this.appiumForMacDriver.get("iPhone Simulator", function() {
              this.appiumForMacDriver.window("1", function() {
                this.appiumForMacDriver.elementByName(this.deviceSpec, function(err,element) {
                  this.appiumForMacDriver.clickElement(element, function() {
                    cb();
                  }.bind(this));
                }.bind(this));
              }.bind(this));
      ...
      msg = "Could not find AppiumForMac binary at " +
        appiumForMacPath;
      logger.error(msg);
      cb(new Error(msg));
    }
...
IOS.prototype.preLaunchSimulatorAndSetOptions = function (cb) {
   this.connectAppiumForMacDriver(cb.bind(this));
};
...
IOS.prototype.postCleanup = function(cb) {
  ...
  if (this.reset) {
    ...
    async.series([
      ...
      function(cb) { this.endAppiumForMac(cb); }.bind(this),
...
IOS.prototype.endAppiumForMac = function(cb) {
  if (this.appiumForMacProcess) {
    logger.info('Terminating Appium for Mac process');
    try { this.appiumForMacProcess.kill('SIGHUP'); }
    catch(err) { logger.warn("Could not kill Appium For Mac."); }
  }
  cb();
};
