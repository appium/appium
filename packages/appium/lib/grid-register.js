...
async function registerToGrid(postOptions, configHolder) {
  try {
    ...
    logger.debug(
      `Appium successfully registered with the the grid on ` +
        hubUri(configHolder.configuration)
    );
  } catch (err) {
    logger.error(
      `An attempt to register with the grid was unsuccessful: ${err.message}`
    );
  }
}
...
function postRequest(configHolder, addr, port, basePath) {
  ...
  if (configHolder.configuration.register !== true) {
    logger.debug(
      `No registration sent (${configHolder.configuration.register} = false)`
    );
    return;
  }
