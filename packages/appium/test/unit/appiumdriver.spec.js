  ...
  describe('instance method', function () {
    ...
    describe('createSession', function () {
      ...
      it('should assign args to property `cliArgs`', async function () {
        ...
        const {value} = await appium.createSession(
          undefined,
          undefined,
          W3C_CAPS
        );
    ...
    describe('deleteSession', function () {
      ...
      it('should remove the session if it is found', async function () {
        let [sessionId] = (await appium.createSession(null, null, W3C_CAPS))
          .value;
        let sessions = await appium.getSessions();
        sessions.should.have.length(1);
      ...
      it("should call inner driver's deleteSession method", async function () {
        const [sessionId] = (await appium.createSession(null, null, W3C_CAPS))
          .value;
        mockFakeDriver
          .expects('deleteSession')
          .once()
    ...
    describe('getSessions', function () {
      ...
      it('should return sessions created', async function () {
        ...
        mockFakeDriver
          ...
          .returns([
            'fake-session-id-1',
            removeAppiumPrefixes(caps1.alwaysMatch),
          ]);
        let [session1Id, session1Caps] = (
          await appium.createSession(null, null, caps1)
        ).value;
        mockFakeDriver
          .expects('createSession')
          .once()
          .returns([
            'fake-session-id-2',
            removeAppiumPrefixes(caps2.alwaysMatch),
          ]);
        let [session2Id, session2Caps] = (
          await appium.createSession(null, null, caps2)
        ).value;
    ...
    describe('attachUnexpectedShutdownHandler', function () {
      ...
      it('should remove session if inner driver unexpectedly exits with an error', async function () {
        let [sessionId] = (
          await appium.createSession(null, null, _.clone(W3C_CAPS))
        ).value; // eslint-disable-line comma-spacing
        _.keys(appium.sessions).should.contain(sessionId);
        appium.sessions[sessionId].eventEmitter.emit(
