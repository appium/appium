    ...
    withSandbox({}, (S) => {
      it('should detect all issues', async function () {
        ...
        S.mocks.checks[0]
          .expects('diagnose')
          .once()
          .returns({ok: true, message: 'All Good!'});
        S.mocks.checks[1]
          .expects('diagnose')
          .twice()
        ...
        S.mocks.checks[2]
          .expects('diagnose')
          .once()
          .returns({ok: false, message: 'Oh No!'});
        S.mocks.checks[3]
          .expects('diagnose')
          .twice()
  ...
  describe(
    ...
    withSandbox({}, (S) => {
      ...
      it('should report success when no fixes are needed', async function () {
        let logStub = stubLog(S.sandbox, log, {stripColors: true});
        (
          await doctor.reportSuccess(
            doctor.toFix.length,
            doctor.toFixOptionals.length
          )
        ).should.equal(true);
        logStub.output.should.equal(
          ['info: Everything looks good, bye!', 'info: '].join('\n')
        );
      });
  ...
  describe(
    ...
    withSandbox({}, (S) => {
      ...
      it('should ask for manual fixes to be applied', async function () {
        ...
        (
          await doctor.reportManualFixes(doctor.toFix, doctor.toFixOptionals)
        ).should.equal(true);
        S.verify();
        logStub.output.should.equal(
          [
