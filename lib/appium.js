...
class AppiumDriver extends BaseDriver {
  ...
  async getDriverForCaps (caps) {
    ...
    const driverSelector = platformsMapping[caps.platformName.toLowerCase()];
    if (driverSelector) {
  ...
  async getDriverVersion (driver) {
    const driversMapping = await this.getDriversMapping();
    if (driversMapping[driver.name] && driversMapping[driver.name].moduleInfo) {
      const {version} = driversMapping[driver.name].moduleInfo;
      if (version) {
        return version;
      }
    }
