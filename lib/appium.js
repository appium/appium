...
class AppiumDriver extends BaseDriver {
  ...
  async createSession (desiredCaps={}, reqCaps, capabilities) {
    if (capabilities) {
      ...
      log.info(`Merged W3C capabilities ${_.truncate(JSON.stringify(capabilities), {length: 50})} into desiredCapabiilities object ${_.truncate(JSON.stringify(desiredCaps), {length: 50})}`);
