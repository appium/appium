...
module.exports = function(app) {
  ...
  var loadWebView = function(browser, urlToLoad, titleToSpin) {
    return loadWebViewBase(app, browser, urlToLoad, titleToSpin);
  };
  ...
  describe('window title', function() {
    ...
    beforeEach(function(done) {
      loadWebView(browser).nodeify(done);
    });
  ...
  describe('webview basics', function() {
    ...
    it('should check if two elements are equals', function(done) {
      Q.all([
        browser.elementById('i_am_an_id'),
        browser.elementByTagName('div')
      ]).then(function(els) {
    ...
    it('should get current url', function(done) {
      browser
        .url()
        .then(function(url) {
          [env.GUINEA_TEST_END_POINT, env.CHROME_GUINEA_TEST_END_POINT]
            .should.include(url);
        }).nodeify(done);
    });
    ...
    it('should move to an arbitrary x-y element and click on it', function(done) {
      browser.elementByLinkText('i am a link')
        .moveTo(5, 15)
        .click()
      .then(function() { return spinTitle("I am another page title", browser); })
      .nodeify(done);
  ...
  describe('implicit wait', function() {
    ...
    it('should set the implicit wait for finding web elements', function(done) {
      browser
        ...
        return browser.setImplicitWaitTimeout(0);
      })
      .nodeify(done);
    });
  });
  ...
  describe('frames', function() {
    ...
    beforeEach(function(done) {
      loadWebView(browser, _testEndpoint(app) + 'frameset.html',
  ...
  describe('iframes', function() {
    ...
    beforeEach(function(done) {
      loadWebView(browser, _testEndpoint(app) + 'iframes.html',
    ...
    describe('within iframe webview', function() {
      it('should be able to get cookies for a page with none', function(done) {
        loadWebView(browser, _testEndpoint(app) + 'iframes.html',
          ...
          return browser.allCookies().should.eventually.have.length(0);
        }).nodeify(done);
      });
    });
  ...
  if (app === "iwebview") {
    describe('https', function() {
      ...
      it('should be able to test self-signed pages', function(done) {
        loadWebView(browser, 'https://selfsigned.buildslave.saucelabs.com',
          "Sauce Labs")
        .nodeify(done);
      });
    });
