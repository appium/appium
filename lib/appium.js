...
class AppiumDriver extends BaseDriver {
  ...
  async getDriversMapping () {
    ...
    for (const [driverClass, driverProps] of _.toPairs(this._driversMapping)) {
      driverProps.moduleInfo = await lookupModuleInfo(driverClass);
    }
  ...
  async getDriverForCaps (caps) {
    ...
    const driversMapping = await this.getDriversMapping();
    // we don't necessarily have an `automationName` capability,
    if (_.isString(caps.automationName)) {
      for (const [driverClass, driverProps] of _.toPairs(driversMapping)) {
        if (!_.isString(driverProps.automationName)) {
          continue;
        }
        if (caps.automationName.toLowerCase() === driverProps.automationName.toLowerCase()) {
          return driverClass;
        }
      }
    }

    const platformsMapping = this.getPlatformsMapping();
    for (const [platformName, driverClassSelector] of _.toPairs(platformsMapping)) {
      if (caps.platformName.toLowerCase() === platformName.toLowerCase()) {
        return await driverClassSelector(caps);
      }
    }
  ...
  async getDriverVersion (driver) {
    const driversMapping = await this.getDriversMapping();
    if (driversMapping[driver]) {
      return driversMapping[driver].moduleInfo.version;
    }
...
async function resolvePackageInfo (modPath) {
  let currentRoot = path.dirname(modPath);
  while (path.dirname(currentRoot) !== currentRoot) {
    const names = await fs.readdir(currentRoot);
    for (const name of names) {
      if (name === 'package.json') {
        try {
          const packageProps = JSON.parse(await fs.readFile(path.resolve(currentRoot, name), 'utf8'));
          if (_.has(packageProps, 'name') && _.has(packageProps, 'version')) {
            return packageProps;
          }
        } catch (ign) {}
      }
    }
    if (currentRoot === path.dirname(currentRoot)) {
      return null;
    }
...
async function lookupModuleInfo (driverClass) {
  for (const [modPath, modInfo] of _.toPairs(require.cache)) {
    if (!_.isPlainObject(modInfo.exports)) {
      continue;
    }
    for (const exportedValue of _.keys(modInfo.exports)) {
      if (_.has(exportedValue, 'name') && exportedValue.name === driverClass.name) {
        const packageInfo = await resolvePackageInfo(modPath);
        if (packageInfo) {
          return packageInfo;
        }
      }
  ...
  throw new Error(`Cannot resolve the original module name for ${driverClass.name}`);
}
