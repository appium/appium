    ...
    withMocks({tp, utils}, (mocks) => {
      ...
      it('diagnose - success', async function () {
        mocks.utils
          .expects('resolveExecutablePath')
          .once()
          .returns('path/to/applesimutils');
        mocks.tp
          .expects('exec')
          .once()
          .returns({stdout: 'vxx.xx.xx', stderr: ''});
        (await check.diagnose()).should.deep.equal({
          ok: true,
          optional: true,
          message:
    ...
    withMocks({tp, utils}, (mocks) => {
      ...
      it('diagnose - success', async function () {
        mocks.utils
          .expects('resolveExecutablePath')
          .once()
          .returns('path/to/ios-deploy');
        mocks.tp.expects('exec').once().returns({stdout: '1.9.4', stderr: ''});
        (await check.diagnose()).should.deep.equal({
          ok: true,
          optional: true,
          message:
            'ios-deploy is installed at: path/to/ios-deploy. Installed version is: 1.9.4',
        });
        mocks.verify();
      });
      it('diagnose - failure', async function () {
