...
import { getDriver, getVersion, AUTOMATION_NAMES, PLATFORM_NAMES } from './driver-utils';
...
const desiredCapabilityConstraints = {
  automationName: {
    presence: false,
    isString: true,
    inclusionCaseInsensitive: AUTOMATION_NAMES,
  },
  platformName: {
    presence: true,
    isString: true,
    inclusionCaseInsensitive: PLATFORM_NAMES,
  },
};
...
class AppiumDriver extends BaseDriver {
  ...
  printNewSessionAnnouncement (driver, caps) {
    const driverVersion = getVersion(driver);
  ...
  async createSession (jsonwpCaps, reqCaps, w3cCapabilities) {
    ...
    try {
      ...
      const InnerDriver = getDriver(desiredCaps);
