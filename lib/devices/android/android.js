...
var errors = require('../../server/errors.js')
  ...
  , appiumBootstrapJar = require('appium-uiautomator').bootstrapJar
  , helperJars = require('appium-adb').jars
  , async = require('async')
...
Android.prototype.initialize = function (opts) {
  ...
  this.remote = null;
  this.windowHandles = [];
  this.curWindowHandle = this.defaultWindow();
  this.didLaunch = false;
  this.launchCb = function () {};
  this.uiautomatorExitCb = function () {};
  this.chromedriverPort = opts.port;
...
Android.prototype.pushStrings = function (cb) {
  ...
  if (!fs.existsSync(this.apkPath)) {
    ...
    var stringsFromApkJarPath = helperJars['strings_from_apk.jar'];
    if (!this.appPackage) {
...
Android.prototype.pushAppium = function (cb) {
  ...
  var binPath = appiumBootstrapJar;
  fs.stat(binPath, function (err) {
    if (err) {
      cb(new Error("Could not find AppiumBootstrap.jar"));
    } else {
