...
|`appWaitActivity`| Activity name/names, comma separated, for the Android activity you want to wait for. By default the value of this capability is the same as for `appActivity`. You must set `appWaitActivity` to wait for activities if your capability has `appActivity` and `appPackage`. The behaviour switch on [`startAppOptions.waitActivity`](https://github.com/appium/appium-adb/blob/master/lib/tools/apk-utils.js#L119). |`SplashActivity`, `SplashActivity,OtherActivity`, `*`, `*.SplashActivity`|
